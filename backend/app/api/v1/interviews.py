...
async def create_session(session_in: InterviewCreate, db: AsyncSession = Depends(get_db)):
...
async def get_application_interviews(application_id: str, db: AsyncSession = Depends(get_db)):
    service = InterviewService(db)
...
async def submit_answer(question_id: str, answer_in: AnswerSubmit, db: AsyncSession = Depends(get_db)):
    service = InterviewService(db)
    simulator = InterviewSimulator(db)
    try:
        answer = await service.submit_answer(question_id, answer_in.answer_text)
        
        # Process Simulation (Evaluate & Generate Follow-up)
